...
issueTrackerApp.controller("IssuesController", function($scope, $modal, $log, $filter, $timeout, pageService, issueService, contactService, statusService, inputFileService, utilService) {
	...
	$scope.doDeleteIssue = function(id, size) {
		...
		modalInstance = $modal.open({
			...
			controller : ModalDeleteIssueInstanceCtrl,
			size : size,
			resolve : {
				item : function() {
					return item;
		...
		modalInstance.result.then(function(selectedItem) {
			...
			$log.info('Modal for delete Issue dismissed at: ' + new Date());
		});
	};
	...
	doNewAction = function() {
		...
		if(isUpdateIssue) {
			issueService.updateIssue(newIssue, function(data, message) {
				//Keep to store issues
				//newPage.storeIssues = $scope.issues || [];
...
var ModalDeleteIssueInstanceCtrl = function($scope, $modalInstance, item, issueService) {

	//For show data in modelDelete.html
	$scope.item = item;

	//For OK button in modelDelete.html
	$scope.ok = function() {
		issueService.deleteIssue(item.id, function(data, message) {
			//alert(message);
			$modalInstance.close(item);
		});
	};

	//For Cancel button in modelDelete.html
...
var ModalTodoInstanceCtrl = function($scope, $modal, $log, $modalInstance, $timeout, $location, item, datePickerService, todoService, issueService, utilService) {
	...
	$scope.doDeleteTodo = function(todo, size) {
		var item = {
				id : todo.id,
				confirm : "todo list"
		},
		modalInstance = $modal.open({
			templateUrl : "view/modalDelete.html",
			controller : ModalDeleteTodoCustomInstanceCtrl,
			size : size,
			resolve : {
				item : function() {
					return item;
		...
		modalInstance.result.then(function(selectedItem) {
			...
			todolist = $scope.todolist;
			for (var i = 0; i < todolist.length; i++) {
				if (todolist[i].id === selectedItem.id) {
					index = i;
					break;
				}
			}
			...
			$log.info('Modal for delete Todo dismissed at: ' + new Date());
		});
	}
...
var ModalDeleteTodoCustomInstanceCtrl = function($scope, $modalInstance, item, todoService) {
	...
	//For OK button in modelDelete.html
	$scope.ok = function() {
		todoService.deleteTodoCustom(item.id, function(data, message) {
			//alert(message);
			$modalInstance.close(item);
		});
	};
