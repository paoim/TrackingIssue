...
issueTrackerApp.controller("TodoController", function($scope, $modal, $log, $filter, $timeout, pageService, todoService, contactService, inputFileService, utilService) {
	var newPage = {
		title : "Todos List",
		isDetailPage : false,
	...
	$scope.doDeleteTodo = function(todo, size) {
		...
		modalInstance = $modal.open({
			...
			controller : ModalDeleteTodoInstanceCtrl,
			size : size,
			resolve : {
				item : function() {
					return item;
...
var ModalDeleteTodoInstanceCtrl = function($scope, $modalInstance, item, todoService) {
	...
	$scope.ok = function() {
		todoService.deleteTodo(item.id, function(data, message) {
			//alert(message);
